...
import { executeCodeSchema, handleExecuteCode, } from './tools/execute-code.js';
import { startSessionSchema, handleStartSession, sendToSessionSchema, handleSendToSession, closeSessionSchema, handleCloseSession, listSessionsSchema, handleListSessions, } from './tools/sessions.js';
import { installPackageSchema, handleInstallPackage, } from './tools/packages.js';
import { saveScriptSchema, handleSaveScript, getScriptSchema, handleGetScript, listScriptsSchema, handleListScripts, searchScriptsSchema, handleSearchScripts, runScriptSchema, handleRunScript, saveAndRunScriptSchema, handleSaveAndRunScript, deleteScriptSchema, handleDeleteScript, } from './tools/scripts.js';
...
export function createServer() {
    ...
    registerTool(server, {
        name: 'execute_code',
        description: 'Run code in a one-shot sandbox. Each call starts a fresh process with no state from previous calls.\n\n' +
            'Args:\n' +
    ...
    registerTool(server, {
        name: 'start_session',
        description: 'Create a persistent REPL session. State (variables, imports, data) persists across send_to_session calls.\n\n' +
            'Args:\n' +
    ...
    registerTool(server, {
        ...
        description: 'Send code to a REPL session. If session_id is omitted, a new session is created automatically.\n\n' +
            'Args:\n' +
    ...
    registerTool(server, {
        name: 'close_session',
        description: 'Kill a REPL session and clean up resources.\n\n' +
            'Args:\n' +
    ...
    registerTool(server, {
        name: 'list_sessions',
        description: 'List all active REPL sessions with language, age, memory usage, and execution count.\n\n' +
    ...
    registerTool(server, {
        ...
        description: 'Install a pip (Python) or npm (Node) package. If session_id is provided, installs into that session and makes it importable. Otherwise installs globally.\n\n' +
            'Args:\n' +
    ...
    registerTool(server, {
        name: 'save_script',
        description: 'Save code as a reusable named script. If a script with the same name exists, it is overwritten.\n\n' +
            'Args:\n' +
    ...
    registerTool(server, {
        name: 'get_script',
        description: 'Retrieve a saved script by name. Returns the code and metadata.\n\n' +
            'Args:\n' +
    ...
    registerTool(server, {
        name: 'list_scripts',
        description: 'List all saved scripts. Optionally filter by language or tag.\n\n' +
            'Args:\n' +
    ...
    registerTool(server, {
        name: 'search_scripts',
        description: 'Search saved scripts by keyword. Matches against name, description, and tags.\n\n' +
            'Args:\n' +
    ...
    registerTool(server, {
        name: 'run_script',
        description: 'Execute a saved script. Runs in a one-shot sandbox with optional arguments.\n\n' +
            'Args:\n' +
    ...
    registerTool(server, {
        ...
        description: 'Save code as a reusable script AND immediately execute it in one step. ' +
    ...
    registerTool(server, {
        name: 'delete_script',
        description: 'Delete a saved script from the library.\n\n' +
            'Args:\n' +
