...
export class SessionManager {
    ...
    async startSession(opts) {
        ...
        if (this.sessions.size >= config.maxSessions) {
            throw new Error(`Maximum ${config.maxSessions} concurrent sessions reached. Close a session first.`);
        }
        ...
        const wrapperFile = opts.language === 'python' ? 'python-repl.py' : 'node-repl.mjs';
        ...
        child.on('close', () => {
            if (this.sessions.has(sessionId)) {
                this.closeSession(sessionId, 'process_exit').catch((err) => logger.error(`Auto-close failed for ${sessionId}: ${err}`));
            }
        });
        // Log session start
        logSessionEvent({
    ...
    async sendToSession(optsOrId, codeArg, timeoutMsArg) {
        ...
        let createdSession;
        const needsAutoCreate = !sessionId || (language && !this.sessions.has(sessionId));
        if (needsAutoCreate) {
            if (!language) {
        ...
        const execPromise = session.pendingExec.then(() => this.doSend(session, code, timeoutMs));
    ...
    async doSend(session, code, timeoutMs) {
        const config = getConfig();
        const executionId = generateExecutionId();
        const timeout = Math.min(timeoutMs ?? config.defaultTimeoutMs, config.maxTimeoutMs);
        this.resetIdleTimer(session);
        ...
        const { stdout, stderr, timed_out } = await this.collectUntilSentinel(session, doneSentinel, timeout);
    ...
    collectUntilSentinel(session, doneSentinel, timeoutMs) {
    ...
    async installPackage(opts) {
        ...
        if (opts.sessionId) {
            ...
            if (session.language !== opts.language) {
                throw new Error(`Language mismatch: session ${opts.sessionId} is ${session.language}, but install requested for ${opts.language}`);
            }
        }
        ...
        if (opts.sessionId && success) {
            const session = this.getSession(opts.sessionId);
            if (opts.language === 'python') {
                await this.sendToSession(session.session_id, `import sys\nif ${JSON.stringify(targetDir)} not in sys.path: sys.path.insert(0, ${JSON.stringify(targetDir)})`, 5_000);
            }
            else {
                await this.sendToSession(session.session_id, `if (!module.paths.includes(${JSON.stringify(join(targetDir, 'node_modules'))})) module.paths.unshift(${JSON.stringify(join(targetDir, 'node_modules'))})`, 5_000);
            }
    ...
    async shutdownAll() {
        ...
        await Promise.allSettled(ids.map((id) => this.closeSession(id, 'manual')));
    }
    // ── Private Helpers ───────────────────────────────────────────────────────
    ...
    resetIdleTimer(session) {
        ...
        session.idleTimer = setTimeout(() => {
            this.closeSession(session.session_id, 'idle_timeout').catch((err) => logger.error(`Idle close failed for ${session.session_id}: ${err}`));
    ...
    async runInstallCommand(cmd, args) {
        return new Promise((resolve) => {
    ...
    parseVersionFromOutput(language, output) {
        if (language === 'python') {
