...
import { loadConfig, validateProviderConfig } from './config.js';
 ...
 */
function createServer(config, startTime) {
    const app = express();
    ...
    app.post('/execute-skill', async (req, res) => {
        ...
        try {
            const result = await agentRef.processProactiveTask(instructions, maxSteps || 10, noTools, requiredTools, skillName, chatId);
            res.json({
                success: result.success,
