 ...
 */
export function relaxSchemaTypes(schema) {
    const relaxed = JSON.parse(JSON.stringify(schema));
    function walk(obj, requiredKeys) {
 ...
 */
function coerceStringBooleans(args) {
 ...
 * Mutates the args object in-place.
 */
export function stripNullValues(args) {
    for (const [key, value] of Object.entries(args)) {
        if (value === null)
            delete args[key];
    }
...
function stripHallucinatedParams(toolName, args) {
 ...
 * Mutates the args object in-place.
 */
export function injectChatId(toolName, args, chatId) {
    if (toolName !== 'telegram_send_message' || !chatId)
        return args;
 ...
 */
export function createToolsFromOrchestrator(orchestratorTools, client, getTrace, getChatId) {
    const tools = {};
    ...
    for (const orchTool of orchestratorTools) {
        ...
        const wrappedTool = tool({
            ...
            execute: async (args) => {
                ...
                const normalizedArgs = injectChatId(orchTool.name, stripHallucinatedParams(orchTool.name, stripNullValues(coerceStringBooleans((args ?? {})))), getChatId());
                ...
                try {
                    const result = await client.executeTool(orchTool.name, normalizedArgs, trace);
                    ...
                    if (trace) {
                        await logger.logToolCallError(trace, orchTool.name, error instanceof Error ? error.message : 'Unknown error');
                    }
                    throw error;
                }
            },
 ...
 */
export function createEssentialTools(client, agentId, getTrace, getChatId) {
    const logger = getTraceLogger();
    return {
        send_telegram: tool({
