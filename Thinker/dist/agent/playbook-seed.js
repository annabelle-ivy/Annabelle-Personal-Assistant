 ...
 */
export async function seedPlaybooks(orchestrator, agentId, trace) {
    ...
    for (const playbook of DEFAULT_PLAYBOOKS) {
        ...
        if (existingSkill) {
            ...
            const existingTc = existingSkill.trigger_config;
            const existingKeywords = existingTc?.keywords ?? [];
            const seedKeywords = playbook.trigger_config.keywords;
            const instructionsChanged = existingSkill.instructions !== playbook.instructions;
            const keywordsChanged = existingKeywords.length !== seedKeywords.length ||
                !seedKeywords.every(k => existingKeywords.includes(k));
            if (instructionsChanged || keywordsChanged) {
                const updateResponse = await orchestrator.executeTool('memory_update_skill', {
        ...
        const response = await orchestrator.executeTool('memory_store_skill', {
            agent_id: agentId,
            name: playbook.name,
            description: playbook.description,
...
//# sourceMappingURL=playbook-seed.js.map