 ...
 */
export function truncateHistoryToolResults(messages, preserveLastN = 2) {
    ...
    if (toolIndices.length <= preserveLastN)
        return messages;
    // Indices to preserve (the last N tool messages)
    const preserveSet = new Set(preserveLastN > 0 ? toolIndices.slice(-preserveLastN) : []);
    const result = [];
        ...
        if (msg.role === 'tool' && !preserveSet.has(i) && Array.isArray(msg.content)) {
            ...
            const truncatedContent = msg.content.map((part) => {
                if (typeof part === 'object' &&
                    ...
                    return {
                        ...part,
                        result: `[${part.toolName}: truncated, was ${resultJson.length} chars]`,
                    };
                }
                return part;
            });
 ...
 */
function getToolCallParts(msg) {
    if (msg.role !== 'assistant' || typeof msg.content === 'string')
        return [];
    if (!Array.isArray(msg.content))
        return [];
    return msg.content.filter((part) => typeof part === 'object' && part !== null && 'type' in part && part.type === 'tool-call');
}
/**
 * Extracts tool-result parts from a tool message's content.
 */
function getToolResultParts(msg) {
    if (msg.role !== 'tool' || !Array.isArray(msg.content))
        return [];
    return msg.content.filter((part) => typeof part === 'object' && part !== null && 'type' in part && part.type === 'tool-result');
}
/**
 ...
 * Does not mutate the input array.
 */
export function repairConversationHistory(messages) {
    if (messages.length === 0)
        return [];
    const repaired = [];
    let repairs = 0;
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (msg.role === 'tool' && repaired.length > 0) {
            ...
            if (matchedParts.length < resultParts.length && matchedParts.length > 0) {
                ...
                for (const p of matchedParts)
                    seenToolCallIds.delete(p.toolCallId);
                continue;
            }
        ...
        if (toolCalls.length > 0) {
            ...
            if (!next || next.role !== 'tool') {
                ...
                for (const tc of toolCalls)
                    seenToolCallIds.delete(tc.toolCallId);
            }
            continue;
        }
