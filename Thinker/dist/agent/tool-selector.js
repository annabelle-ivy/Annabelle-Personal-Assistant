 ...
 */
function expandPattern(pattern, allToolNames) {
    if (!pattern.includes('*')) {
        return [pattern];
    }
 ...
 */
function getEffectiveGroups(mcpMetadata) {
 ...
 */
function getEffectiveKeywordRoutes(mcpMetadata) {
    ...
    if (mcpMetadata) {
        for (const [mcpName, meta] of Object.entries(mcpMetadata)) {
            if (meta.keywords && meta.keywords.length > 0) {
                ...
                if (!hasRoute) {
                    const escaped = meta.keywords.map((k) => k.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
 ...
 */
export function selectToolsForMessage(message, allTools, mcpMetadata) {
