 ...
 */
function applyToolCap(tools, coreNames, scores, cap) {
 ...
 */
export async function selectToolsWithFallback(message, allTools, embeddingSelector, mcpMetadata) {
    if (embeddingSelector?.isInitialized()) {
        try {
            ...
            if (stats) {
                logger.info(`Tool selection: method=embedding+regex, embedding=${stats.selectedCount}, regex added=${regexAdded}, total=${Object.keys(merged).length}/${stats.totalTools}, topScore=${stats.topScore.toFixed(3)}`);
            }
