 ...
 */
function nameToNegativeId(name) {
    let hash = 0;
...
export class SkillLoader {
     ...
     */
    async parseSkillFile(skillMdPath, dirName) {
        let content;
        try {
        ...
        try {
            ...
            logger.warn(`Failed to parse YAML in ${dirName}/SKILL.md: ${error instanceof Error ? error.message : error}`);
            return null;
        }
        ...
        const executionPlan = Array.isArray(meta.execution_plan) ? meta.execution_plan : undefined;
        return {
