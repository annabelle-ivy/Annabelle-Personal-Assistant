...
import { createTrace } from '../tracing/context.js';
 ...
 */
function extractTextTurns(messages) {
    const result = [];
...
export class Agent {
    ...
    // Rate limiting
    lastApiCallTime = 0;
    minApiCallIntervalMs = 1000; // 1 second minimum between calls
    // Circuit breaker
    consecutiveErrors = 0;
    ...
    constructor(config) {
        ...
        this.sessionStore = new SessionStore(config.sessionsDir, config.thinkerAgentId, config.sessionConfig);
    ...
    async initialize() {
        ...
        const dynamicTools = createToolsFromOrchestrator(orchestratorTools, this.orchestrator, () => this.currentTrace, getChatId);
        // Create essential tools
        const essentialTools = createEssentialTools(this.orchestrator, this.config.thinkerAgentId, () => this.currentTrace, getChatId);
    ...
    async refreshToolsIfNeeded() {
        try {
            ...
            const getChatId = () => this.currentChatId || this.config.defaultNotifyChatId;
            const dynamicTools = createToolsFromOrchestrator(freshOrchestratorTools, this.orchestrator, () => this.currentTrace, getChatId);
            const essentialTools = createEssentialTools(this.orchestrator, this.config.thinkerAgentId, () => this.currentTrace, getChatId);
            this.tools = { ...dynamicTools, ...essentialTools };
     ...
     */
    async selectRelevantHistory(userMessage, allMessages) {
        ...
        try {
            ...
            for (const { pairStart } of scored) {
                if (selectedOlderMessages.length >= budget)
                    break;
            ...
            logger.info(`[history-select] Selected ${result.length}/${allMessages.length} messages ` +
     ...
     */
    async buildContext(chatId, userMessage, trace) {
        ...
        const memories = await this.orchestrator.retrieveMemories(this.config.thinkerAgentId, userMessage, 5, trace);
        ...
        return {
            systemPrompt,
            conversationHistory: truncateHistoryToolResults(repairConversationHistory(await this.selectRelevantHistory(userMessage, state.messages.slice(-30))), 2),
    ...
    async processMessage(message) {
        ...
        try {
            ...
            const buildRetryMessages = (stepsCount, userText, errorContext) => {
                const msgs = [
                    ...context.conversationHistory,
                    { role: 'user', content: userText ?? message.text },
                ];
                if (errorContext) {
                    msgs.push({ role: 'assistant', content: errorContext });
                    msgs.push({ role: 'user', content: 'Please try again using only the tools available to you.' });
                }
            ...
            try {
                ...
                const isToolCallError = toolErrorMsg.includes('Failed to call a function') ||
                            ...
                            if (collectedResults.length > 0) {
                                ...
                                try {
                                    ...
                                    const rawText = collectedResults
                                        ...
                                        .join('\n\n');
                                    result = { text: rawText, steps: [], toolCalls: [], toolResults: [], finishReason: 'stop', usage: { promptTokens: 0, completionTokens: 0, totalTokens: 0 }, warnings: undefined, response: { id: '', timestamp: new Date(), modelId: '' }, logprobs: undefined, reasoning: undefined, reasoningDetails: [], files: [], sources: [], request: {}, responseMessages: [] };
                                }
                            }
                        }
...
- Do NOT pretend you can look something up — be honest that tools are temporarily unavailable if you truly have no data to answer with.`;
            ...
            await this.logger.logLLMCallComplete(trace, providerInfo.provider, providerInfo.model, promptTokens, completionTokens, llmDuration);
            ...
            let responseText;
            let recoveredTools = [];
            const shouldAttemptRecovery = result.finishReason === 'stop' &&
            ...
            if (shouldAttemptRecovery) {
                ...
                if (leak.detected) {
                    ...
                    const recovery = await recoverLeakedToolCall(leak.toolName, leak.parameters, selectedTools);
                    if (recovery.success) {
                        ...
                        if (leak.preamble) {
                            ...
                            try {
                                ...
                                this.costMonitor?.recordUsage(summary.usage?.promptTokens || 0, summary.usage?.completionTokens || 0);
            ...
            const noToolsUsed = result.steps.every((step) => !step.toolCalls?.length) &&
            ...
            if (this.config.sessionConfig.enabled) {
                try {
                    ...
                    const turnMessages = hasStructuredResponse
                        ? [{ role: 'user', content: message.text }, ...result.response.messages]
                        : undefined;
                    await this.sessionStore.saveTurn(message.chatId, message.text, responseText, result.steps
                        .flatMap((step) => step.toolCalls?.map((tc) => tc.toolName) || []), { prompt: promptTokens, completion: completionTokens }, turnMessages);
                    ...
                    if (this.sessionStore.shouldCompact(message.chatId)) {
                        ...
                        const compactionResult = await this.sessionStore.compact(message.chatId, textMessages, compactionModel);
            ...
            const resultToolNames = result.steps.flatMap((step) => step.toolCalls?.map((tc) => tc.toolName) || []);
            const capturedToolNames = capturedSteps.flatMap((step) => step.toolCalls?.map((tc) => tc.toolName) || []);
            ...
            const coreSet = new Set(CORE_TOOL_NAMES);
            const stickyToolsThisTurn = toolsUsed.filter((t) => t !== '(text-only fallback)' && !coreSet.has(t));
            ...
            if (this.config.sendResponseDirectly) {
                ...
                await this.orchestrator.storeConversation(this.config.thinkerAgentId, message.text, responseText, undefined, trace);
            }
            else {
                await this.logger.logResponseSent(trace, message.chatId, responseText.length);
            }
     ...
     */
    async processProactiveTask(taskInstructions, maxSteps = 10, noTools, requiredTools, skillName, chatId) {
...
Complete the task step by step, using your available tools. When done, provide a brief summary of what you accomplished.`;
            ...
            const memories = await this.orchestrator.retrieveMemories(this.config.thinkerAgentId, taskInstructions, 5, trace);
            ...
            try {
                result = await generateText({
                    ...
                    ...(selectedTools ? { tools: selectedTools, toolChoice: 'auto' } : {}),
                    maxSteps,
                    temperature: this.config.temperature,
                ...
                const isToolCallError = genMsg.includes('Failed to call a function') ||
            ...
            // Record token usage in cost monitor
            this.costMonitor?.recordUsage(result.usage?.promptTokens || 0, result.usage?.completionTokens || 0);
            ...
            try {
                await this.orchestrator.storeFact(this.config.thinkerAgentId, `Skill execution summary: ${responseText.substring(0, 500)}`, 'pattern', trace);
            }
            catch (memError) {
     ...
     */
    async runFactExtraction(chatId) {
        ...
        if (state.lastExtractionAt && state.lastExtractionAt >= state.lastActivity)
            return;
        ...
        try {
            ...
            const userIdentity = this.config.userName
                ...
                : undefined;
            const facts = await extractFactsFromConversation(model, recentMessages, knownFactStrings, this.config.factExtraction.confidenceThreshold, userIdentity);
            if (facts.length === 0) {
            ...
            for (const fact of facts) {
                const success = await this.orchestrator.storeFact(this.config.thinkerAgentId, fact.fact, fact.category);
                if (success)
                    stored++;
            }
            state.lastExtractionAt = Date.now();
            ...
            logger.warn('[fact-extraction] Failed (non-fatal):', error instanceof Error ? error.message : error);
        }
    }
    /**
     * Clean up old conversation states
     */
    cleanupOldConversations(maxAgeMs = 30 * 60 * 1000) {
        const now = Date.now();
     ...
     */
    resumeFromCostPause(resetWindow = false) {
        if (!this.costMonitor) {
