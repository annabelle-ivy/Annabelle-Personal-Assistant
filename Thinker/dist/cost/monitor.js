...
function emptyBucket(minuteTimestamp) {
...
export class CostMonitor {
    ...
    constructor(config) {
        ...
        this.buckets = Array.from({ length: TOTAL_BUCKETS }, (_, i) => emptyBucket(now - (TOTAL_BUCKETS - 1 - i) * 60_000));
    }
    // ─── Public API ─────────────────────────────────────────────────
    /**
    ...
    advanceWindow() {
        ...
        if (gap <= 0)
            return; // still in the same minute
